...
test('Create offender in Ndelius', async ({ page }) => {
  ...
  // Create a new person
  const person: Person = deliusPerson();
...
test('create and add a sentence plan against the offender', async ({ request }) => {
  ...
  const postResponse = await request.post(
    'https://sentence-plan-api-dev.hmpps.service.justice.gov.uk/coordinator/plan',
    {
      headers: {
  ...
  const putResponse = await request.put(
    `https://sentence-plan-api-dev.hmpps.service.justice.gov.uk/plans/associate/${planId}/${globalCRN}`,
    {
      headers: {
        Authorization: `Bearer ${token}`,
        'Content-Type': 'application/json',
