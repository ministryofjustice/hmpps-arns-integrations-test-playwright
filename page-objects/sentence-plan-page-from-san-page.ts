...
export class SentencePlanfromSanPage {
  ...
  async checkPageTitle() {
    ...
    await expect(latestPage).toHaveTitle(
  ...
  async clickAboutPageAfterNavigatingToSPWithoutOasysStep() {
    const aboutLink = newTabGlobal.getByRole('link', { name: 'About', exact: false });
  ...
  async checkAboutPageDisplaysCorrectInfoForIncompleteSan() {
    ...
    const warning = newTabGlobal.getByLabel('Warning');
    await expect(warning).toHaveCount(1);
    const incompleteInfo = newTabGlobal.locator('h2.govuk-heading-m').first();
    ...
    const employmentAndEducationSection = newTabGlobal.getByLabel('Employment and education ,');
    await employmentAndEducationSection.click();
    const highScoringInfoContent = newTabGlobal.locator('#assessment-accordion-highScoring-content-1');
    const highScoringInfoText = await highScoringInfoContent.textContent();
    expect(highScoringInfoText).toContain(highScoringInfoContentDetails);
    await employmentAndEducationSection.click();
    ...
    const accommodationSection = newTabGlobal.getByLabel('Accommodation , , Show this');
    await accommodationSection.click();
    const lowScoringInfoContent = newTabGlobal.locator('#assessment-accordion-lowScoring-content-1');
    const lowScoringInfoText = await lowScoringInfoContent.textContent();
    expect(lowScoringInfoText).toContain(lowScoringInfoContentDetails);
  }
}
