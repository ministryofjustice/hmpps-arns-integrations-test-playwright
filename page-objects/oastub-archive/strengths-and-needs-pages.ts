...
export class StrengthsAndNeedsLandingPage {
  ...
  async validationErrorDisplaysOnDataPrivacyScreen() {
    await expect(
      newTabGlobal.locator('#main-content div').filter({ hasText: 'There is a problem Confirm' }).nth(3)
    ).toBeVisible();
    await expect(newTabGlobal.locator('#privacy_screen_declaration-error')).toBeVisible();
  }

  async tickConfirmBox() {
    await newTabGlobal.locator('#privacy_screen_declaration').check();
  }
  ...
  async fillInOptionalInfo() {
    await newTabGlobal.locator('#employment_history_unknown_details').fill('test');
  }
  ...
  async checkDrugMotivationErrorDisplays() {
    await expect(
      newTabGlobal.locator('#main-content div').filter({ hasText: 'There is a problem Select if' }).nth(3)
    ).toBeVisible();
  ...
  async tickPersonalRelationshipsRiskOfHarm() {
    await newTabGlobal
      .locator('#personal_relationships_community_practitioner_analysis_risk_of_serious_harm-2')
      .check();
  }
  ...
  async fillReasonForOffence() {
    await newTabGlobal.locator('#offence_analysis_reason').fill('This is why this took place.');
  }
  ...
  async fillDetailsAboutNoRiskOfSeriousHarm() {
    await newTabGlobal.locator('#offence_analysis_risk_no_details').fill('No risk of serious harm.');
  }
  ...
  async clickLatestPreviousVersion() {
    ...
    const s = day.toLocaleDateString('en-GB', {
      day: 'numeric',
      month: 'long',
    });
    // let twoDigitDay:string = ('0'+s).slice(-2);
    const previousVersionsDate: string = 'View   assessment from' + ' ' + s;
  ...
  async checkPreviousAssessment() {
    ...
    const s = today.toLocaleDateString('en-GB', {
      day: 'numeric',
      month: 'long',
      year: 'numeric',
    });
