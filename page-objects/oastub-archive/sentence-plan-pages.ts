...
export class SentencePlanPage {
  ...
  async checkPageTitleDataPrivacyScreen() {
    ...
    await expect(newTab).toHaveTitle(
      'Remember to close any other applications before starting an appointment - Sentence plan'
    );
    return newTab;
  }

  async checkPageTitleDataPrivacyScreenAfterNavigatingToSP() {
    await expect(newTabGlobal).toHaveTitle(
      'Remember to close any other applications before starting an appointment - Sentence plan'
    );
  }
  ...
  async checkStrenghtsAndProtectiveFactorsAreInPlace() {
    await expect(
      newTabGlobal.getByText('There are strengths or protective factors related to this area')
    ).toBeVisible();
  }
  ...
  async checkStrenghtsAndProtectiveFactorsAreNotInPlace() {
    await expect(
      newTabGlobal.getByText('There are no strengths or protective factors related to this area')
    ).toBeVisible();
  }

  async checkMotivationResponseDisplaysAsMissingInfo() {
    await expect(
      newTabGlobal.locator('#main-content > div > div.govuk-grid-column-two-thirds > details > div > ul > li')
  ...
  async fillInGoalTitle() {
    await newTabGlobal.locator('#goal-input-autocomplete').fill('Test Accomodation goal');
  }
  ...
  async fillInStepTitle() {
    await newTabGlobal.locator('#step-description-1').fill('Accomodation step title');
  }

  async fillInSecondStepTitle() {
    await newTabGlobal.locator('#step-description-2').fill('Second step title');
  }
  ...
  async fillInNewGoalTitle() {
    await newTabGlobal.locator('#goal-input-autocomplete').fill('Test New Accomodation goal');
  }
  ...
  async checkExpectedStepIsListedWhenViewStepsElementIsToggled() {
    await expect(
      newTabGlobal.locator(
        '#goal-summary-card-1 > div > div.govuk-summary-card__content > details > div > table > tbody > tr:nth-child(1) > td:nth-child(2)'
      )
  ...
  async checkRemovedStepIsNotShowingWhenViewStepsElementIsToggled() {
    await expect(
      newTabGlobal.locator(
        '#goal-summary-card-1 > div > div.govuk-summary-card__content > details > div > table > tbody > tr:nth-child(2) > td:nth-child(2)'
      )
    ).toHaveCount(0);
  }
  ...
  async checkNewStepIsStoredCorrectly() {
    await expect(
      newTabGlobal.locator(
        '#goal-summary-card-1 > div > div.govuk-summary-card__content > div.govuk-details__text_inactive > table > tbody > tr:nth-child(2) > td:nth-child(2)'
      )
  ...
  async checkExistingStepHasNotBeenDeleted() {
    await expect(
      newTabGlobal.locator(
        '#goal-summary-card-1 > div > div.govuk-summary-card__content > div.govuk-details__text_inactive > table > tbody > tr > td:nth-child(2)'
      )
    ).toBeVisible();
  }
  ...
  async fillInIDoNotAgreeDetails() {
    await newTabGlobal.locator('#does-not-agree-details').fill('I do not agree');
  }
  ...
  async fillInCouldNotAnswerDetails() {
    await newTabGlobal.locator('#could-not-answer-details').fill('I could not answer');
  }

  async fillInNotesAboutAgreeingPlan() {
    await newTabGlobal.locator('#notes').fill('Test notes about agreeing the plan.');
  }
  ...
  async checkAddStepsValidationErrorDisplays() {
    await expect(newTabGlobal.locator('div').filter({ hasText: 'There is a problem Add steps' }).nth(3)).toBeVisible();
  }
  ...
  async checkNumberOfGoalsToWorkOnNowIsCorrect() {
    await expect(newTabGlobal.getByRole('link', { name: 'Goals to work on now (2)' })).toBeVisible();
  }

  async checkNumberOfFutureGoalsIsCorrect() {
    await expect(newTabGlobal.getByRole('link', { name: 'Future goals (1)' })).toBeVisible();
  }
  ...
  async updateStepStatustoInProgress() {
    await newTabGlobal
      .getByRole('row', { name: 'Probation practitioner test 1' })
      .getByLabel('Status')
  ...
  async checkStepStatusIsSetToInProgress() {
    await expect(
      newTabGlobal.getByRole('row', { name: 'Probation practitioner test 1' }).getByRole('strong')
  ...
  async checkPlanCreationUpdateIsUnique() {
    await expect(newTabGlobal.locator('p').filter({ hasText: PLAN_CREATED_DATA }).getByRole('strong')).toHaveCount(1);
  }
  ...
  async checkPlanCreationIsNotOverwritten() {
    ...
    await expect(
      newTabGlobal
        .locator('p')
        .filter({ hasText: 'Plan agreed on ' + todayDate })
        .getByRole('strong')
    ).toHaveCount(0);
  }
  ...
  async checkBannerDisplaysForIncompleteAssessment() {
    await expect(newTabGlobal.getByLabel('Warning')).toHaveCount(1);
    await expect(newTabGlobal.locator('h2.govuk-heading-m').first()).toHaveText(
      'Some areas have incomplete information'
    );
  }

  async checkBannerDoesntDisplayForCompleteAssessment() {
    await expect(newTabGlobal.getByLabel('Warning')).toHaveCount(0);
  }

  async clickShowAllSectionsAccordion() {
    await newTabGlobal
      .locator('#assessment-accordion-highScoring')
      .getByRole('button', { name: 'Show all sections' })
      .click();
  }

  async clickCreateAGoalLinkFromWithinSection() {
    await newTabGlobal.locator('#assessment-accordion-highScoring-content-1 > p.goal-link > a').click();
  }
  ...
  async checkThereAreNoHighOrLowScoringAreas() {
    await expect(newTabGlobal.locator('#main-content > div > div > p:nth-child(9)')).toHaveText(
      'No high-scoring areas at the moment.'
    );
    await expect(newTabGlobal.locator('#main-content > div > div > p:nth-child(11)')).toHaveText(
      'No low-scoring areas at the moment.'
    );
  }
  ...
  async checkNoInfoAvailableDisplays() {
    ...
    await expect(newTabGlobal.getByText('No information is available yet')).toBeVisible();
  }
  ...
  async checkCompletedAssessmentLowScoringSectionDisplaysCorrectly() {
    ...
    const lowScoringInfoAccordion = newTabGlobal.locator('#assessment-accordion-lowScoring');
    await expect(lowScoringInfoAccordion).toBeVisible();
    const lowScoringInfoSections = [newTabGlobal.getByLabel('Accommodation'), newTabGlobal.getByLabel('Alcohol use')];
  ...
  async checkAgreementDataCorrectlyDisplaysInPlanHistory() {
    await expect(newTabGlobal.locator('#main-content > div > div > p:nth-child(2) > strong')).toContainText(
      'Agreement updated'
    );
    await expect(newTabGlobal.locator('#main-content > div > div > p:nth-child(3)')).toContainText(
      'did not agree to this plan.'
    );
    await expect(newTabGlobal.locator('#main-content > div > div > div:nth-child(4)')).toContainText('I do not agree');
    await expect(newTabGlobal.locator('#main-content > div > div > p:nth-child(6) > strong')).toContainText(
      'Plan created'
    );
    await expect(newTabGlobal.locator('#main-content > div > div > p:nth-child(7)')).toContainText(
      'could not agree to this plan.'
    );
    await expect(newTabGlobal.locator('#main-content > div > div > div:nth-child(8)')).toContainText(
      'I could not answer'
    );
    await expect(newTabGlobal.locator('#main-content > div > div > div.plan-additional-note')).toContainText(
      'Test notes about agreeing the plan.'
    );
  }
